...
readonly VERSION_FILE="${root}/snesdriver/version.py"
...
replace_recorded_version() {
  local wanted=$1
  local scratch
  scratch=$(mktemp "${TMPDIR:-/tmp}/snesdriver-version-XXXXXX")
...
replace_cited_version() {
  local wanted=$1
  local scratch
  scratch=$(mktemp "${TMPDIR:-/tmp}/snesdriver-citation-XXXXXX")
