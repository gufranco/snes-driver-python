...
This member consumes two others, both as git submodules on the import path
rather than as published packages. What a driver routine is written in is the
65816, which `mos65xx-python` disassembles, and which addresses reach the part is
decided by the layout, which `snes-mapper` reads. Both sit at the repository root
named after themselves, so what this is built on is visible the moment the
repository is opened.

That choice has the price the section above names, and it is paid deliberately.
The download-zip link on the repository page produces a checkout that cannot run.

It reads rather than models. Nothing here is executed and nothing here is a
claim about what a coprocessor answers, which belongs to the members that model
those parts. What it establishes is what a shipped cartridge sends, taken from
the cartridge's own code in the order the console runs it.
