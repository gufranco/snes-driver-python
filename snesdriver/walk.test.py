...
class StoppingTest(unittest.TestCase):
    def test_a_walk_stops_where_the_routine_returns(self):
        steps = list(walk.through(assembled(STORE_LONG, RETURN, STORE_LONG), 0))

        self.assertEqual(len(steps), 2)

    def test_and_where_it_jumps_away(self):
        steps = list(walk.through(assembled(STORE_LONG, (0x4C, 0x00, 0x80), STORE_LONG), 0))

        self.assertEqual(len(steps), 2)

    def test_a_walk_stops_after_the_number_of_instructions_it_was_allowed(self):
        code = assembled(*([STORE_LONG] * 40))
        steps = list(walk.through(code, 0, limit=5))

        self.assertEqual(len(steps), 5)

    def test_a_walk_that_runs_off_the_end_stops_there(self):
...
class ReachTest(unittest.TestCase):
    def test_a_long_store_reports_the_bank_and_address_it_reaches(self):
    ...
    def test_a_long_load_is_told_apart_from_a_store(self):
    ...
    def test_an_instruction_that_reaches_nowhere_long_says_so(self):
        steps = list(walk.through(assembled((0xA9, 0x06), RETURN), 0, narrow=True))

        self.assertIsNone(steps[0].bank)

    def test_an_immediate_load_reports_the_value_it_carries(self):
        steps = list(walk.through(assembled((0xA9, 0x06), RETURN), 0, narrow=True))

        self.assertEqual(steps[0].immediate, 0x06)

    def test_a_wide_immediate_carries_both_of_its_bytes(self):
        steps = list(walk.through(assembled((0xA9, 0x34, 0x12), RETURN), 0, narrow=False))

        self.assertEqual(steps[0].immediate, 0x1234)

    def test_an_instruction_carrying_no_immediate_says_so(self):
        steps = list(walk.through(assembled(STORE_LONG, RETURN), 0))

        self.assertIsNone(steps[0].immediate)

    def test_a_step_prints_as_the_instruction_it_stands_for(self):
        steps = list(walk.through(assembled(STORE_LONG, RETURN), 0))
...
class AddressTest(unittest.TestCase):
    def test_an_instruction_reaching_nowhere_long_has_no_address(self):
        steps = list(walk.through(assembled((0xA9, 0x06), RETURN), 0, narrow=True))
