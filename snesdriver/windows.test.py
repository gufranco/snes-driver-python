...
class ChoosingTest(unittest.TestCase):
    def test_a_layout_the_part_has_a_window_for_is_found(self):
        self.assertIsNotNone(windows.window_for("dsp", "lorom"))

    def test_a_layout_it_has_none_for_answers_nothing(self):
        self.assertIsNone(windows.window_for("dsp", "exhirom"))

    def test_a_part_it_does_not_know_is_refused(self):
        with self.assertRaises(windows.UnknownPart):
            windows.window_for("nonsense", "lorom")

    def test_the_parts_it_knows_are_listed_in_the_refusal(self):
...
class GuessingTest(unittest.TestCase):
    def test_a_part_with_no_window_for_the_layout_is_passed_over(self):
        reaches = [(0x00, 0x6000)] * 5

        self.assertEqual(windows.busiest("hirom", reaches), "dsp")

    def test_the_busiest_window_in_a_run_of_addresses_is_the_one_chosen(self):
        reaches = [(0x30, 0x8000)] * 20 + [(0x00, 0x2140)] * 3

        self.assertEqual(windows.busiest("lorom", reaches), "dsp")

    def test_a_run_that_reaches_no_window_chooses_nothing(self):
        self.assertIsNone(windows.busiest("lorom", [(0x00, 0x2140)] * 5))

    def test_and_an_empty_run_chooses_nothing(self):
