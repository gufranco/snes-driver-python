...
class VersionTest(unittest.TestCase):
    def test_a_version_is_recorded(self):
        self.assertTrue(version.VERSION)

    def test_it_reads_as_three_numbers(self):
        self.assertRegex(version.VERSION, r"^\d+\.\d+\.\d+([-+].*)?$")

    def test_the_release_script_writes_the_field_this_file_holds(self):
        script = (ROOT / "scripts" / "set-version.sh").read_text()

        self.assertIn("snesdriver/version.py", script)

    def test_nothing_else_in_the_package_carries_a_version(self):
        elsewhere = [
            path.name
